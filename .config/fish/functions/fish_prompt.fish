function fish_prompt
  ...
  if not set -q __fish_prompt_hostname
    set -g __fish_prompt_ip (ifconfig | sed -n '/.inet /{s///;s/ .*//;p;}' | sed -e 's/127\.0\.0\.1//' | tr '\n' ' ' | xargs)
  ...
  # Top
  echo -n $cyan$USER$normal at $yellow$__fish_prompt_hostname \($__fish_prompt_ip\)$normal in $bred(prompt_pwd)$normal
  __fish_git_prompt
